...
class Cart extends AppModel {
	...
	public function addToCart($product, $qty = 1, $mod = null) {
		...
		$_SESSION['cart.sum'] = isset($_SESSION['cart.sum']) ? $_SESSION['cart.sum'] +  ( $price * $_SESSION['cart.currency']['value'] ) : $price * $_SESSION['cart.currency']['value'] ;
	}
	...
	public static function recalc($curr) {
		...
		foreach ($_SESSION['cart'] as $key => $value) {
			if ($_SESSION['cart.currency']['base']) {
				$_SESSION['cart'][$key]['price'] = 	$_SESSION['cart'][$key]['price'] * $curr->value;
			} else {
